...
//   useEffect(() => {
//     const SHRINK_ON = 60;  // 내려갈 때 이 값 넘으면 shrink
//     const SHRINK_OFF = 10; // 위로 올라올 때 이 값 아래면 원복

//     const onScroll = () => {
//       const y = window.scrollY;

//       setIsShrink((prev) => {
//         // 아직 shrink가 아닌 상태에서 충분히 내려가면 shrink로 전환
//         if (!prev && y > SHRINK_ON) return true;

//         // shrink 상태에서 거의 맨 위로 올라오면 원복
//         if (prev && y < SHRINK_OFF) return false;

//         // 임계 구간에서는 상태 유지(토글 방지)
//         return prev;
//       });
//     };

//     window.addEventListener("scroll", onScroll, { passive: true });
//     onScroll(); // 처음 렌더 때도 상태 맞추기

//     return () => window.removeEventListener("scroll", onScroll);
//   }, []);

//   return (
//     <header className={`siteHeader ${isShrink ? "shrink" : ""}`}>
